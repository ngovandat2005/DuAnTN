...
function Cart() {
  ...
  const getErrorMessage = (error, defaultMessage = "Có lỗi xảy ra") => {
  ...
  const showApiError = (error, defaultMessage = "Có lỗi xảy ra") => {
    const msg = getErrorMessage(error, defaultMessage);

    if (
      msg.includes("Không đủ số lượng tồn") ||
      msg.includes("Khong du so luong ton")
    ) {
      message.error("Sản phẩm k đủ số lượng");
      return;
    }

    if (msg.includes("Hàng đã hết") || msg.includes("Hang da het")) {
      message.error("Sản phẩm đã hết hàng");
      return;
    }

    if (
      msg.includes("Số lượng mua phải lớn hơn 0") ||
      msg.includes("So luong mua phai lon hon 0")
    ) {
  ...
  useEffect(() => {
    const fetchCart = async () => {
      try {
        ...
        console.log(
          "📍 API:",
          config.getApiUrl(`api/gio-hang-chi-tiet/${customerId}`),
        );
        ...
        if (response.data && response.data.length > 0) {
          ...
          response.data.forEach((item, index) => {
            ...
            if (item.sanPhamChiTiet) {
              console.log(
                `📦 Item ${index + 1} có sanPhamChiTiet:`,
                item.sanPhamChiTiet,
              );
              if (item.sanPhamChiTiet.giaBan) {
                console.log(
                  `💰 Item ${index + 1} sanPhamChiTiet.giaBan:`,
                  item.sanPhamChiTiet.giaBan,
                );
              }
              if (item.sanPhamChiTiet.giaBanGiamGia) {
                console.log(
                  `🎯 Item ${index + 1} sanPhamChiTiet.giaBanGiamGia:`,
                  item.sanPhamChiTiet.giaBanGiamGia,
                );
              }
            }
            ...
            const tenSanPham =
              item.tenSanPham || item.sanPhamChiTiet?.sanPham?.tenSanPham;
        ...
        message.error(`Không lấy được giỏ hàng: ${getErrorMessage(error)}`);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleQuantityChange = async (id, newQuantity) => {
    ...
    try {
      ...
      console.log(
        "📍 API:",
        config.getApiUrl(`api/gio-hang-chi-tiet/cap-nhat`),
      );
      ...
      const msg =
        typeof raw === "string"
          ? raw
          : raw?.message || error?.message || "Cập nhật thất bại";

      if (
        msg.includes("Không đủ số lượng tồn") ||
        msg.includes("Khong du so luong ton")
      ) {
        openErrorModal("Sản phẩm không đủ số lượng trong kho.");
        return;
      }

      if (msg.includes("Hàng đã hết") || msg.includes("Hang da het")) {
        openErrorModal("Sản phẩm đã hết hàng.");
        return;
      }
      ...
      if (
        ...
        openErrorModal("Số lượng không hợp lệ.");
        return;
      }
  ...
  const handleDeleteAll = async () => {
    try {
      ...
      const response = await axios.delete(
        config.getApiUrl(`api/gio-hang-chi-tiet/xoa-tat-ca/${customerId}`),
      );

      // ✅ SỬA: HTTP 204 (No Content) cũng là thành công
      if (response.status === 200 || response.status === 204) {
        console.log(
          "✅ Đã xóa toàn bộ giỏ hàng thành công (Status:",
          response.status,
          ")",
        );
  ...
  const handleDeleteItem = async (idGioHangChiTiet) => {
    ...
    try {
      console.log(
        "🗑️ Đang xóa sản phẩm với ID giỏ hàng chi tiết:",
        idGioHangChiTiet,
      );

      // ✅ SỬA: Sử dụng API mới xóa theo ID
      // Làm sạch ID (bỏ ":1" nếu có)
      const cleanId = String(idGioHangChiTiet).split(":")[0];
      const response = await axios.delete(
        config.getApiUrl(`api/gio-hang-chi-tiet/xoa/${cleanId}`),
      );

      // ✅ SỬA: Xử lý cả 200 và 204
      if (response.status === 200 || response.status === 204) {
        console.log(
          "✅ Xóa sản phẩm thành công (Status:",
          response.status,
          "), đang refresh giỏ hàng...",
        );
        ...
        try {
          ...
          if (
            ...
            console.log(
              "✅ Đã refresh giỏ hàng từ backend, số lượng:",
              (refreshResponse.data || []).length,
            );
          }
        } catch (refreshError) {
      ...
      showApiError(error, "Xóa sản phẩm thất bại");
    }
  };
  ...
  const total = cart.reduce((sum, item) => {
    ...
    if (item.giaBan !== undefined && item.giaBanGiamGia !== undefined) {
      ...
      console.log(
        `🆕 Cấu trúc mới - giaBan: ${item.giaBan}, giaBanGiamGia: ${item.giaBanGiamGia}`,
      );

      // ✅ SỬA: Logic đúng - kiểm tra null trước khi so sánh
      const hasDiscount =
        item.giaBanGiamGia &&
        item.giaBanGiamGia > 0 &&
        item.giaBanGiamGia < item.giaBan;
      console.log(
        `🔍 Kiểm tra: giaBanGiamGia && giaBanGiamGia > 0 && giaBanGiamGia < giaBan = ${item.giaBanGiamGia} && ${item.giaBanGiamGia} > 0 && ${item.giaBanGiamGia} < ${item.giaBan} = ${hasDiscount}`,
      );

      giaBan = hasDiscount ? item.giaBanGiamGia : item.giaBan;
      source = "giaBan_giaBanGiamGia";
      console.log(
        `💰 Kết quả: giaBan = ${giaBan} (${hasDiscount ? "khuyến mãi" : "giá gốc"})`,
      );
    } else if (item.sanPhamChiTiet && item.sanPhamChiTiet.giaBan) {
      // Cấu trúc cũ: item.sanPhamChiTiet.giaBan
      console.log(
        `📦 Cấu trúc cũ - sanPhamChiTiet.giaBan: ${item.sanPhamChiTiet.giaBan}, giaBanGiamGia: ${item.sanPhamChiTiet.giaBanGiamGia}`,
      );
    ...
    console.log(
      `💰 Item ${item.id}: giá=${giaBan}, số lượng=${item.soLuong || 1}, thành tiền=${itemTotal}, nguồn=${source}`,
    );
  ...
  return (
    <>
      <Modal
        ...
        footer={[
          <Button key="ok" type="primary" onClick={closeErrorModal}>
            Đóng
          </Button>,
        ]}
        title={
          <span style={{ color: "red", fontWeight: "bold" }}>Thông báo</span>
        }
      >
      ...
      <div className="gx-cart-root gx-cart-full-bg">
        <div className="gx-cart-title-row">
          <ShoppingCartOutlined
            style={{ fontSize: 32, color: "#ff6600", marginRight: 12 }}
          />
                  ...
                  <tr key={item.id} className="gx-cart-row">
                      ...
                      <img
                        src={
                          ...
                          (() => {
                            ...
                            if (
                              item.sanPhamChiTiet &&
                              item.sanPhamChiTiet.sanPham &&
                              item.sanPhamChiTiet.sanPham.imanges
                            ) {
                              // Cấu trúc cũ: item.sanPhamChiTiet.sanPham.imanges
                              imagePath =
                                item.sanPhamChiTiet.sanPham.imanges.split(
                                  ",",
                                )[0];
                            ...
                            if (imagePath) {
                              ...
                              if (imagePath.startsWith("http")) {
                                ...
                                return config.getApiUrl(
                                  `api/images/${encodeURIComponent(imagePath.substring(1))}`,
                                );
                              } else {
                    ...
                    <td className="gx-cart-price">
                      ...
                      {(() => {
                        ...
                        if (
                          ...
                          console.log(
                            `🆕 Cấu trúc mới: giaBan=${giaBan}, giaBanGiamGia=${giaBanGiamGia}, hasDiscount=${hasDiscount}`,
                          );
                        } else if (
                          item.sanPhamChiTiet &&
                          item.sanPhamChiTiet.giaBan
                          ...
                          hasDiscount =
                            ...
                            giaBanGiamGia < giaBan;
                          console.log(
                            `📦 Cấu trúc cũ: giaBan=${giaBan}, giaBanGiamGia=${giaBanGiamGia}, hasDiscount=${hasDiscount}`,
                          );
                        } else {
                          // ✅ SỬA: Không set giá = 0, mà tìm giá từ nơi khác
                          console.warn(
                            "⚠️ Cấu trúc dữ liệu không xác định, tìm giá từ nơi khác:",
                            item,
                          );
                          ...
                          if (item.gia) {
                            ...
                            hasDiscount = false;
                            console.log(
                              `🔍 Tìm thấy giá từ trường gia: ${giaBan}`,
                            );
                          } else if (item.price) {
                            giaBan = item.price;
                            giaBanGiamGia = item.discountPrice || 0;
                            hasDiscount =
                              giaBanGiamGia > 0 && giaBanGiamGia < giaBan;
                            console.log(
                              `🔍 Tìm thấy giá từ trường price: ${giaBan}, discountPrice: ${giaBanGiamGia}`,
                            );
                          } else {
                            // Cuối cùng mới set = 0
                            giaBan = 0;
                            ...
                            hasDiscount = false;
                            console.error(
                              "❌ Không tìm thấy giá từ bất kỳ trường nào!",
                            );
                          }
                        }
                        ...
                        console.log(
                          `🔍 DEBUG hiển thị giá - Item ${item.id}:`,
                          {
                            giaBan,
                            giaBanGiamGia,
                            hasDiscount,
                        ...
                        if (hasDiscount && giaBanGiamGia > 0) {
                          console.log(
                            `🎯 Hiển thị giá khuyến mãi: gốc=${giaBan}, khuyến mãi=${giaBanGiamGia}`,
                          );
                          return (
                            <div>
                              <div
                    ...
                    <td className="gx-cart-quantity">
                      ...
                      <Button
                        size="small"
                        onClick={() =>
                          handleQuantityChange(item.id, item.soLuong + 1)
                        }
                      >
                        +
                      </Button>
                      ...
                      {(() => {
                        ...
                        if (
                          ...
                          giaBan = hasDiscount
                            ...
                            : item.giaBan;
                          console.log(
                            `🆕 Tính thành tiền - Cấu trúc mới: giaBan=${item.giaBan}, giaBanGiamGia=${item.giaBanGiamGia}, hasDiscount=${hasDiscount}, sử dụng=${giaBan}`,
                          );
                        } else if (
                          item.sanPhamChiTiet &&
                          item.sanPhamChiTiet.giaBan
                          ...
                          giaBan = hasDiscount
                            ...
                            : item.sanPhamChiTiet.giaBan;
                          console.log(
                            `📦 Tính thành tiền - Cấu trúc cũ: giaBan=${item.sanPhamChiTiet.giaBan}, giaBanGiamGia=${item.sanPhamChiTiet.giaBanGiamGia}, hasDiscount=${hasDiscount}, sử dụng=${giaBan}`,
                          );
                        } else {
                          // ✅ SỬA: Không set giá = 0, mà tìm giá từ nơi khác
                          console.warn(
                            "⚠️ Tính thành tiền - Cấu trúc dữ liệu không xác định, tìm giá từ nơi khác:",
                            item,
                          );
                          ...
                          if (item.gia) {
                            giaBan = item.gia;
                            console.log(
                              `🔍 Tính thành tiền - Tìm thấy giá từ trường gia: ${giaBan}`,
                            );
                          } else if (item.price) {
                            giaBan = item.price;
                            console.log(
                              `🔍 Tính thành tiền - Tìm thấy giá từ trường price: ${giaBan}`,
                            );
                          } else {
                            // Cuối cùng mới set = 0
                            giaBan = 0;
                            console.error(
                              "❌ Tính thành tiền - Không tìm thấy giá từ bất kỳ trường nào!",
                            );
                          }
                        }

                        return (
                          (giaBan * (item.soLuong || 1)).toLocaleString() + "đ"
                        );
                      })()}
                    </td>
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          if (item.id) {
                            ...
                            itemId = item.id;
                            console.log(
                              "🔍 Sử dụng ID giỏ hàng chi tiết:",
                              itemId,
                            );
                          } else {
                            console.error(
                              "❌ Không tìm thấy ID giỏ hàng chi tiết:",
                              item,
                            );
                            message.error(
                              "Không thể xác định sản phẩm cần xóa!",
                            );
                            return;
                          }

                          console.log(
                            "🚀 Gọi handleDeleteItem với ID giỏ hàng chi tiết:",
                            itemId,
                          );
